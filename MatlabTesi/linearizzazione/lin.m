...
elseif all(re_hover < -tol)
    ...
    status_hover = 'SISTEMA STABILE SEMPLICEMENTE';
end
...
% autovalori = diag(D);
% W = inv(V);
% % 3. Calcolo Matrice di Partecipazione P (modulo)
...
%% =========================================================================
% CASE 2: CRUISE (VOLO ORIZZONTALE) - 28 STATI
% =========================================================================
...
elseif all(re_cruise < -tol)
    ...
    status_cruise = 'SISTEMA STABILE SEMPLICEMENTE';
end
...
% Trova gli indici degli autovalori nulli
...
for k = 1:length(null_indices)
    ...
    % 1. Stampa l'intestazione del blocco per questo autovalore
    fprintf('\n--- Analisi Autovalore %d ---\n', k);
    fprintf('Valore: %.4e\n', autoval_c(idx));
    fprintf('Autovettore associato:\n');
    
    % 2. Stampa l'autovettore riga per riga
    % Usiamo V_c(:, idx) per prendere la colonna specifica
    v_attuale = V_c(:, idx);
    
    for i = 1:length(v_attuale)
        % Stampa indice dello stato e valore della componente
        fprintf('  Stato x(%2d): %8.4f\n', i, v_attuale(i));
    end
end

% %% =========================================================================
% % ANALISI DELLA PARTECIPAZIONE AGGREGATA 
% % =========================================================================
% fprintf('\n--- CALCOLO MATRICE DI PARTECIPAZIONE AGGREGATA ---\n');
% 
% % 1. Ricalcolo P_norm 
...
% P_macro(4, :) = sum(P_norm(10:12, :), 1); % Vel. Angolare (pqr)
% P_macro(5, :) = sum(P_norm(13:26, :), 1); % Attuatori (Motori e Servi)
% P_macro(6, :) = sum(P_norm(27:28, :), 1); % Integratori
% 
% % Per rendere il grafico più pulito, ordiniamo gli autovalori dal 
% % più lento (Re più vicina a 0) al più veloce (Re più negativa)
% [~, sort_idx] = sort(real(autovalori), 'descend');
% autovalori_sorted = autovalori(sort_idx);
% P_macro_sorted = P_macro(:, sort_idx);
