...
\begin{frame}
    ...
    \begin{minipage}{0.8\textwidth}
        ...
        {\Huge \textbf{Validazione Statistica}} \\
        \large Campagna di Stress-Test Monte Carlo ($N=300$) \\
...
\begin{frame}{Metodologia di Validazione Estesa}
    \begin{alertblock}{Stress-Test Monte Carlo}
        Il sistema è stato testato su 300 scenari indipendenti per verificare la robustezza del controllo rispetto a incertezze iniziali e mismatch tra modello fisico e modello di controllo interno.
    \end{alertblock}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \textbf{\large Incertezze Iniziali ($\sigma$)}
            \begin{itemize}
                \item \textbf{Posizione:} $\pm 2.0$ m
                \item \textbf{Velocità:} $\pm 3.0$ m/s
                \item \textbf{Assetto:} $\pm 15^\circ$
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
% --- SLIDE 2: DECOLLO ---
\begin{frame}{Scenario 1: Decollo e Controllo Verticale}
    \textbf{Analisi:} Transizione Idle $\to$ Hover in configurazione tricottero puro.
    ...
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            \begin{block}{Risultati (Successo 100\%)}
                \centering
                RMSE Quota $\approx 2.8 \times 10^{-6}$ m
            \end{block}
            \textbf{Punti di Forza:}
            \begin{itemize}
                \item \textbf{Disaccoppiamento:} In assenza di vento relativo, il mixing dei motori non subisce disturbi aerodinamici.
                \item \textbf{Lift-off:} La spinta supera il peso ($mg \approx 65.6$N) con un transitorio smorzato.
            \end{itemize}
        \end{column}
        
        \begin{column}{0.5\textwidth}
            \centering
            % Inserire qui il grafico a due piani (Quota + Spinta)
            \includegraphics[width=\linewidth, height=5cm, keepaspectratio]{montecarlo/takeoff.png}
            \captionof{figure}{\footnotesize \textbf{Tracking Quota e Spinta:} Sovrapposizione di 100 simulazioni. Si nota il picco di spinta al lift-off e la successiva stabilizzazione al peso nominale.}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Scenario 2: Recupero in Hovering (Recovery)}
    \textbf{Obiettivo:} Annullamento della velocità orizzontale residua per il rientro in hovering stazionario.
    
    \begin{columns}[T]
        \begin{column}{0.45\textwidth}
            \textbf{Analisi del Comportamento:}
            \begin{itemize}
                \item \textbf{Frenata Aerodinamica:} Il sistema usa il beccheggio ($\theta$) per generare resistenza e annullare $V_x$.
                \item \textbf{Accoppiamento:} Si osservano oscillazioni dovute allo scambio di energia tra assetto e spinta dei rotori.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{center}
...
% --- SLIDE 4: CROCIERA ---
\begin{frame}{Scenario 3: Crociera e Tracking di Velocità}
    \textbf{Analisi:} Verifica del bilanciamento aerodinamico e stabilità asintotica.
    
    \vspace{0.3cm}
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            \textbf{Performance (99\% Successo):}
            \begin{itemize}
                \item \textbf{RMSE $V_x$:} \textcolor{green!60!black}{\textbf{$7.5 \times 10^{-6}$ m/s}}
                \item \textbf{RMSE Quota:} \textcolor{green!60!black}{\textbf{$1.1 \times 10^{-4}$ m}}
            \end{itemize}
            \vspace{0.2cm}
            \small
            \textit{Il debug del feedforward (fattore 2 nel $C_L$) ha eliminato il drift di velocità, stabilizzando il drone a 25 m/s.}
        \end{column}
        
        \begin{column}{0.5\textwidth}
            \begin{alertblock}{Analisi Outlier (Run 87)}
                Unico crash ($1\%$) dovuto a stallo di gravità.
                L'assetto è mantenuto, ma la portanza è insufficiente a sostenere il peso ($RMSE_Z = 183$m).
            \end{alertblock}
        \end{column}
    \end{columns}
    \vspace{0.2cm}
    ...
    \centering
    \includegraphics[width=0.8\linewidth]{montecarlo/cruise.png}
\end{frame}

% --- SLIDE 5: SINTESI ---
\begin{frame}{Sintesi della Validazione}
    \centering
    \Large Affidabilità confermata sull'intero inviluppo operativo.
    ...
    \begin{table}[]
        ...
        \begin{tabular}{l c l p{4cm}}
            \toprule
            \textbf{Fase} & \textbf{Esito} & \textbf{Stato} & \textbf{Conclusione Tecnica} \\
            \midrule
            Decollo & 100\% & \textcolor{green}{Ottimo} & PID verticale robusto. \\
            Recupero & 96\% & \textcolor{orange}{Critico} & Accoppiamento non-lineare forte. \\
            Crociera & 99\% & \textcolor{green}{Ottimo} & Feedforward validato. \\
            \bottomrule
        \end{tabular}
    \end{table}
    \vspace{0.3cm}
    \normalsize
    \textit{I punti di forza risiedono nella precisione del volo livellato; la criticità risiede nella transizione dinamica per la perdita di portanza alare.}
\end{frame}
