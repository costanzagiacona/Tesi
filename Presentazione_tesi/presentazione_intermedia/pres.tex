...
\begin{frame}{Architettura Gerarchica: Full Cascaded SMC}
    Si adotta un controllo a cascata basato interamente sulla teoria dei \textbf{Sistemi a Struttura Variabile} per garantire robustezza deterministica.
...
\begin{frame}{Inversione Cinematica: Dal Loop Esterno all'Interno}
    Le forze virtuali generate dagli SMC di posizione ($F_{x,req}, F_{y,req}$) devono essere tradotte in angoli di assetto, sfruttando la natura sottosituata del drone.
    ...
    \begin{columns}[c]
        \begin{column}{0.48\textwidth}
            \begin{block}{Generazione Rollio ($\phi_{des}$)}
                ...
                \end{equation*}
                Mappa la forza laterale richiesta in un'inclinazione coordinata.
            \end{block}
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Generazione Beccheggio ($\theta_{des}$)}
                ...
                \end{equation*}
                Trasforma la necessità di spinta longitudinale in angolo d'attacco.
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{itemize}
        ...
        \item \textbf{Output:} Setpoint angolari $\mathbf{A}_{des}$ per l'SMC interno.
        \item \textbf{Vincolo:} L'efficacia dell'inversione dipende dalla rapidità del loop interno nel raggiungere $\phi_{des}$ e $\theta_{des}$.
    \end{itemize}
\end{frame}
...
\begin{frame}{Inner Loop: Stabilizzazione d'Assetto}
    ...
    \begin{itemize}
        \item \textbf{Performance:} Successo nel \textbf{100\%} dei test di stress.
        \item \textbf{Precisione:} Errore d'assetto medio di soli $1.3 \times 10^{-3}$ rad.
        \item \textbf{Vantaggio:} Il drone ignora le raffiche di vento finché sono inferiori al guadagno $K$.
    \end{itemize}
\end{frame}
...
% --- SLIDE 3: OUTER LOOP SMC (MATEMATICA) ---
\begin{frame}{Outer Loop: Guidance Robusta (SMC)}
    In crociera, lo SMC compensa le incertezze del modello aerodinamico (portanza e resistenza) in tempo reale.
    
    \vspace{0.3cm}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{block}{SMC Quota ($z$)}
                Superficie: $s_z = \dot{e}_z + \lambda_z e_z$
                \begin{equation*}
                    F_{z,req} = \underbrace{F_{nom}}_{\text{FF + Weight}} - \underbrace{m K_z \tanh(s_z/\Phi_z)}_{\text{Azione Robusta}}
                \end{equation*}
                Elimina il drift di quota causato da errori nel $C_L$.
            \end{block}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \begin{block}{SMC Velocità ($v_x$)}
                Superficie: $s_v = v_{des} - v$
                \begin{equation*}
                    F_{x,req} = \underbrace{D_{x}(v)}_{\text{Drag FF}} + \underbrace{K_v \tanh(s_v/\Phi_v)}_{\text{Azione Robusta}}
                \end{equation*}
                Garantisce il mantenimento dei $25$ m/s nonostante variazioni di Drag.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
% --- SLIDE 4: INNER LOOP SMC (VIRTUAL SURFACES) ---
\begin{frame}{Inner Loop: Thrust Vectoring via SMC}
    I comandi d'assetto sono generati forzando lo stato sulle sliding surfaces definite. Questo rende il "Rudder" e gli "Ailerons" virtuali insensibili alle variazioni d'inerzia $J$.

    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Robustezza Deterministica:} Successo del 100\% nei test di volo avanzato.
        \item \textbf{Smoothing:} L'uso di un \textit{boundary layer} previene vibrazioni strutturali nel tilt-mechanism dei motori anteriori.
    \end{itemize}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{l l l}
            \toprule
            \textbf{Canale} & \textbf{Superficie ($s$)} & \textbf{Azione ($u$)} \\
            \midrule
            Pitch ($\theta$) & $\dot{e}_\theta + \lambda_\theta e_\theta$ & $K_\theta \tanh(s_\theta/\Phi)$ \\
            Roll ($\phi$) & $\dot{e}_\phi + \lambda_\phi e_\phi$ & $K_\phi \tanh(s_\phi/\Phi)$ \\
            Yaw ($\psi$) & $\dot{e}_\psi + \lambda_\psi e_\psi$ & $K_\psi \tanh(s_\psi/\Phi)$ \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
% ===========================================================
% 5. VALIDAZIONE ROBUSTEZZA (MONTE CARLO) - VERSIONE FINALE
% ===========================================================
\section{Validazione e Analisi Statistica}

% --- SLIDE 1: METODOLOGIA ---
\begin{frame}{Metodologia di Validazione Estesa}
    \begin{alertblock}{Stress-Test Monte Carlo su Full Cascaded SMC}
        Il sistema è stato validato su 300 simulazioni per quantificare la robustezza deterministica del controllo \textbf{Sliding Mode} a fronte di incertezze iniziali ($\pm 15^\circ$ assetto, $\pm 2$m posizione) e mismatch del modello aerodinamico.
    \end{alertblock}

    \vspace{0.4cm}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{\large Il Campione ($N=300$)}
            \begin{itemize}
                \item \textbf{Scenario 1:} Decollo (Takeoff)
                \item \textbf{Scenario 2:} Recupero (Recovery)
                \item \textbf{Scenario 3:} Crociera (Cruise)
            \end{itemize}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \textbf{\large Indicatori di Performance}
            \begin{itemize}
                \item \textbf{Affidabilità:} Tasso di successo globale.
                \item \textbf{Precisione:} RMSE su tutti i 12 stati.
                \item \textbf{Qualità:} Analisi del chattering (RMSE ratei).
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
% --- SLIDE 2: DECOLLO ---
\begin{frame}{Scenario 1: Decollo e Controllo Verticale (SMC)}
    \textbf{Analisi:} Validazione dello SMC con protezione del mixer per lo Yaw.
    
    \vspace{0.2cm}
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            \begin{block}{Risultati (Successo 100\%)}
                \centering
                RMSE Quota $\approx 2.6 \times 10^{-5}$ m \\
                RMSE Yaw ($\Psi$) $\approx 1.2 \times 10^{-12}$ rad
            \end{block}
            \textbf{Punti di Forza:}
            \begin{itemize}
                \item \textbf{Safe Mixer:} La protezione sulla spinta minima impedisce divergenze di Yaw al lift-off.
                \item \textbf{Tracking:} Precisione millimetrica sulla quota grazie al termine di feedforward compensato dallo SMC.
            \end{itemize}
        \end{column}
        
        \begin{column}{0.5\textwidth}
            \centering
            \includegraphics[width=\linewidth, height=5cm, keepaspectratio]{montecarlo/takeoff.png}
        \end{column}
    \end{columns}
\end{frame}

% --- SLIDE 3: RECOVERY ---
\begin{frame}{Scenario 2: Recupero in Hovering (SMC Recovery)}
    \textbf{Analisi:} Stabilizzazione da assetti estremi e annullamento velocità.
    
    \begin{columns}[T]
        \begin{column}{0.45\textwidth}
            \textbf{Performance ($100\%$ Successo):}
            \begin{itemize}
                \item \textbf{Reiezione Disturbi:} RMSE $V_x \approx 2.4 \times 10^{-12}$ m/s nonostante perturbazioni iniziali di $\pm 1.5$ m/s.
                \item \textbf{Pulizia Controllo:} Ratei angolari ($p,q,r$) con RMSE $< 10^{-11}$ rad/s, confermando l'efficacia dello strato limite $\Phi$.
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{center}
                \includegraphics[width=\linewidth, height=5cm, keepaspectratio]{montecarlo/recovery_V.png}
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
...
% --- SLIDE 4: CRUISE ---
\begin{frame}{Scenario 3: Crociera e Tracking (SMC Cruise)}
    \textbf{Analisi:} Integrazione SMC con la portanza alare ($V_a = 25$ m/s).
    
    \begin{columns}[c]
        \begin{column}{0.55\textwidth}
            \begin{block}{Risultati (Successo 100\%)}
                \centering
                RMSE Velocità $\approx 1.5 \times 10^{-4}$ m/s \\
                RMSE Quota $\approx 4.5 \times 10^{-4}$ m
            \end{block}
            \textbf{Conclusioni Crociera:}
            \begin{itemize}
                \item \textbf{Robustezza allo Stallo:} Lo SMC corregge attivamente le carenze del modello di portanza nominale.
                \item \textbf{Stabilità d'Assetto:} Beccheggio ($\Theta$) mantenuto con errore $< 10^{-4}$ rad durante il volo livellato.
            \end{itemize}
        \end{column}
        
        \begin{column}{0.45\textwidth}
            \begin{alertblock}{Deriva Laterale}
                RMSE $Y \approx 44$ m evidenzia l'assenza di un loop di controllo cross-track (coordinata $Y$ libera).
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
...
% --- SLIDE 5: SINTESI ---
\begin{frame}{Sintesi della Validazione}
    \centering
    \Large Affidabilità totale raggiunta tramite controllo robusto.
    ...
    \begin{table}[]
        ...
        \begin{tabular}{l c l p{4.5cm}}
            \toprule
            \textbf{Fase} & \textbf{Esito} & \textbf{Stato} & \textbf{Feedback Tecnico} \\
            \midrule
            Decollo & 100\% & \textcolor{green}{Ottimo} & Zero divergenze di Yaw. \\
            Recupero & 100\% & \textcolor{green}{Ottimo} & Chattering eliminato su $p,q,r$. \\
            Crociera & 100\% & \textcolor{green}{Ottimo} & Successo totale dopo integrazione SMC. \\
            \bottomrule
        \end{tabular}
    \end{table}
    \vspace{0.3cm}
    \normalsize
    \textit{L'architettura Cascaded SMC garantisce il 100\% di sopravvivenza del velivolo in tutto l'inviluppo operativo, superando i limiti del controllo lineare.}
\end{frame}
...
\begin{frame}{Conclusioni e Sviluppi Futuri}
    ...
    \begin{itemize}
        ...
        \item Sintesi controllore robusto per il volo verticale.
        \item Sintesi controllore robusto per il volo orizzontale.
    \end{itemize}
