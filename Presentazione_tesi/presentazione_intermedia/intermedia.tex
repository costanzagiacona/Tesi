...
% Slide 2: Variabili di Stato (Rigido)
\begin{frame}{Variabili di Stato: Corpo Rigido ($x_1 - x_{12}$)}
...
% Slide 3: Variabili di Stato (Attuatori)
\begin{frame}{Variabili di Stato: Dinamica Attuatori ($x_{13} - x_{26}$)}
    ...
    \end{table}
    \normalsize
    \textit{Nota: Ogni attuatore è modellato come un sistema del secondo ordine.}
\end{frame}
...
% Slide 6: Dinamica Traslazionale
\begin{frame}{Dinamica Traslazionale (Newton)}
    ...
    \vspace{0.5em}
    \textbf{Nota critica:} La forza di spinta $F_{Thrust}$ dipende non linearmente dagli angoli di tilt attuali ($x_{13}, \dots$) e non istantaneamente dagli ingressi $u$.
\end{frame}

% Slide 7: Dinamica Rotazionale
\begin{frame}{Dinamica Rotazionale (Eulero)}
    ...
    \begin{itemize}
        \item $M_{thrust}$: Momenti generati dalla spinta vettorizzata.
        \item $M_{gyro}$: Effetti giroscopici dei rotori (significativi ad alti RPM).
        \item $M_{drag}$: Resistenza aerodinamica rotazionale.
    \end{itemize}
\end{frame}

% Slide 8: Modello Attuatori
\begin{frame}{Dinamica degli Attuatori}
    Per aumentare la fedeltà del modello, i motori e i servi sono sistemi del \textbf{secondo ordine}. Esempio per il tilt ($\theta_i$):
    ...
    \begin{alertblock}{Implicazione di Controllo}
...
\begin{frame}{Setup Monte Carlo: Recovery Test 3D}
    ...
    \end{itemize}
    \vspace{0.2cm}
    \textit{Obiettivo: Validare la capacità di "Homing" isotropico.}
\end{frame}
...
\begin{frame}{Analisi Differenziale Assi Planari (X, Y)}
    ...
    \begin{alertblock}{Diagnosi del "Professore"}
        \begin{itemize}
            \item \textbf{Caso Tipico (75\% dei casi):} Il controllo su X è \textbf{chirurgico} (errori in $\mu m$), superiore alla Y.
...
\begin{frame}{MMA 3: Attuatori}
    ...
    \begin{equation}
        \begin{cases}
            \delta_{1} = \alpha_{base} + u_{\theta} - u_{\phi} \\
            \delta_{2}  = \alpha_{base} + u_{\theta} + u_{\phi}
        \end{cases}
    \end{equation}
...
\begin{frame}
    \vfill
    \centering
    {\Huge \textbf{Analisi Monte Carlo per il volo orizzontale}}
    \vfill
\end{frame}

%% --- Slide 2: Obiettivo e Setup ---
\begin{frame}{Setup Sperimentale: Case 3 "noWF"}
    Analisi delle prestazioni in Crociera ($N=100$ run) con configurazione aggiornata.
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Obiettivo:} Verificare se la robustezza è mantenuta rimuvendo il filtro/peso (noWF).
        \item \textbf{Target:} $V_x = \SI{25}{m/s}$, $Z = \SI{-10}{m}$.
        \item \textbf{Risultato Atteso:} Mantenimento della stabilità e tracciamento dei riferimenti.
    \end{itemize}
\end{frame}

% --- Slide 3: Robustezza (Il Grande Successo) ---
\begin{frame}{Statistiche di Convergenza}
    ...
    \begin{itemize}
        \item \textbf{Sicurezza:} 100 su 100 simulazioni completate senza crash.
        \item \textbf{Significato:} Il sistema è estremamente robusto alle condizioni iniziali perturbate. Non soffre più di divergenze numeriche o stalli catastrofici.
    \end{itemize}
\end{frame}

% --- Slide 4: Canale Velocità (Il Problema) ---
\begin{frame}{Analisi Velocità Longitudinale ($V_x$)}
    Si rileva un'anomalia critica nel tracciamento della velocità target (\SI{25}{m/s}).
    ...
    \begin{table}
        ...
        \begin{tabular}{lccc}
            \toprule
            \textbf{Metrica} & \textbf{Valore} & \textbf{Dev. Std.} & \textbf{Diagnosi} \\
            \midrule
            RMSE Medio & \SI{10.35}{m/s} & \SI{0.002}{m/s} & \textcolor{red}{\textbf{Bias Sistematico}} \\
            RMSE Mediana & \SI{10.35}{m/s} & - & Errore Costante \\
            \bottomrule
        \end{tabular}
    \end{table}
    
    \begin{alertblock}{Diagnosi del "Professore"}
        L'errore non è stocastico (la deviazione standard è nulla). Il drone si "inchioda" stabilmente a una velocità errata ($\approx \SI{14.65}{m/s}$ oppure $\approx \SI{35.35}{m/s}$).
        \textbf{Causa probabile:} Errore di Trim del Pitch. Il controllore non comanda un angolo di picchiata sufficiente per raggiungere i 25 m/s, oppure satura un integratore.
    \end{alertblock}
\end{frame}

% --- Slide 5: Canale Verticale (Z) ---
\begin{frame}{Analisi Quota ($Z$)}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                \item \textbf{RMSE Medio:} \SI{0.44}{\ milli\meter}
                \item \textbf{RMSE Mediana:} \SI{0.18}{\ milli\meter}
                \item \textbf{Max Error:} \SI{9}{\ milli\meter}
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{block}{Valutazione}
                \textcolor{green!60!black}{\textbf{Perfetto.}}
                Il disaccoppiamento funziona: l'incapacità di raggiungere la velocità target non compromette la tenuta della quota. Il drone vola "lento ma preciso in Z".
            \end{block}
        \end{column}
    \end{columns}
\end{frame}

% --- Slide 6: Canale Laterale (Y) ---
\begin{frame}{Analisi Laterale ($Y$)}
    Conferma del comportamento in anello aperto (Open Loop).
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{RMSE Medio:} \SI{97.6}{\meter}
        \item \textbf{Picco Max:} \SI{341}{\meter}
        \item \textbf{Ratei $p, r$:} Praticamente nulli ($\approx 10^{-17}$ rad/s).
    \end{itemize}
    
    \vspace{0.5cm}
    \textit{Il drone mantiene un assetto perfettamente livellato (ratei nulli) ma deriva lateralmente a causa dell'assenza di una legge di guida di posizione Y.}
\end{frame}

% --- Slide 7: Sintesi Finale ---
\begin{frame}{Conclusioni Case 3 "noWF"}
    \centering
    \begin{tabular}{l c l}
        \toprule
        \textbf{Aspetto} & \textbf{Stato} & \textbf{Note} \\
        \midrule
        \textbf{Robustezza} & \cellcolor{green!20}\textbf{OTTIMA} & 100\% Successo. Sistema affidabile. \\
        \textbf{Quota ($Z$)} & \cellcolor{green!20}\textbf{ECCELLENTE} & Errore sub-millimetrico. \\
        \textbf{Velocità ($V_x$)} & \cellcolor{red!20}\textbf{CRITICA} & \textbf{Bias di 10 m/s.} Tuning necessario. \\
        \textbf{Laterale ($Y$)} & \cellcolor{yellow!20}\textbf{OPEN LOOP} & Deriva attesa (Navigazione mancante). \\
        \bottomrule
    \end{tabular}

    \vspace{0.5cm}
    \begin{block}{Azione Correttiva Richiesta}
        Il problema prioritario non è più la stabilità, ma la prestazione su $V_x$. 
        È necessario verificare il \textbf{Feedforward del Pitch} o il guadagno integrale sul canale velocità per eliminare l'errore a regime.
    \end{block}
\end{frame}
