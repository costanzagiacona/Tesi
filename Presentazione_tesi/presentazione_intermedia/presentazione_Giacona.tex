...
% --- METADATI ---
\title{Modellazione dinamica e design del controllo per un VTOL innovativo}
...
\begin{frame}{Momenti Attivi: Controllo del Velivolo}
    ...
    \vspace{0.3cm}
    \textbf{Momento di Spinta ($\bm{M}_{th}$)}
    \begin{equation}
        \bm{M}_{\text{th}} = \sum_{i=1}^{3} (\bm{r}_i \times \bm{F}_{th_{i}})
    \end{equation}

    \vspace{0.4cm}
    \textbf{Momento Torcente ($\bm{M}_{tor}$)}
    \begin{equation}
...
\begin{frame}{Analisi di Stabilità per il Controllo di Quota}
    ...
    Sia $V = \frac{1}{2}\sigma^2$. La dinamica perturbata è $\dot{\sigma} = -k \operatorname{sgn}(\sigma) - \Delta(t)$, con $|\Delta(t)| \le \Delta_{max}$.
...
\begin{frame}{Regolarizzazione e Boundary Layer}
    ...
    Si introduce un \textbf{boundary layer} $\Phi$ definendo l'approssimazione:
    \begin{equation*}
        u_{smc} = -k \tanh\left( \frac{\sigma}{\Phi} \right)
    \end{equation*}
    ...
    \begin{block}{Ultimate Boundedness}
        Il sistema non converge più asintoticamente a zero, si parla di stabilità pratica. L'errore residuo è dettato dal bilancio tra la pendenza del controllo ($k/\Phi$) e l'incertezza $\Delta(t)$.
    \end{block}
\end{frame}
...
\begin{frame}{Motor Mixing Algorithm: Thrust e Imbardata}
    Si ipotizza di ripartire equamente la spinta $F_{z,req}$ fra i tre motori.
    \begin{equation}
        \begin{cases}
...
\begin{frame}{Outer Loop: Quota e Velocità}
    ...
    \begin{columns}[T]
        % Colonna Quota
        \begin{column}{0.48\textwidth}
            \begin{block}{Controllo Quota ($z$) $\rightarrow$ Pitch ($\theta_{ref}$)}
                Il controllore di quota $z$ calcola un angolo di beccheggio desiderato $\theta_{des}$ che verrà poi inseguito dal loop interno:
...
\begin{frame}{Motor Mixing Algorithm: Thrust}
...
\begin{frame}
    \vfill
    \centering
    {\Huge \textbf{Analisi di Stabilità tramite\\ Linearizzazione}}
    \vfill
\end{frame}
...
\begin{frame}{Confronto Dinamico: Hovering vs Crociera}
    \begin{columns}[t]
        ...
        \begin{column}{0.48\textwidth}
            \centering
            \textbf{\alert{Hovering ($n=26$)}}
            \vspace{0.2cm}
            \begin{itemize}
                \item \textbf{Trim:} $z = -10$ m, $v = 0$.
                ...
                \item \textbf{Polo Critico:} $\lambda = -0.3358$ (decadimento esponenziale).
            \end{itemize}
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            \centering
            \textbf{\alert{Crociera ($n=28$)}}
            \vspace{0.2cm}
            \begin{itemize}
                \item \textbf{Trim:} $v_x = 25$ m/s, $z = -10$ m.
                \item \textbf{Controllo:} PID con azione integrale su $v_x$ e $z$.
                \item \textbf{Stabilità:} \textbf{Semplice}.\\
                Presenza di $\lambda = 0$.
                \item \textbf{Analisi Spettrale:} Autovettore $v_{\lambda=0}$ associato solo a $x_1$ (invarianza per traslazione).
            \end{itemize}
        \end{column}
    \end{columns}
...
\begin{frame}{Metodologia di Validazione}
    \begin{alertblock}{Stress-Test stocastico ($N=300$)}
        Il sistema è stato sottoposto a 100 simulazioni per ogni scenario per quantificare la robustezza dei controllori scelti a fronte di incertezze iniziali su assetto e posizione.
    \end{alertblock}

    \vspace{0.3cm}
            \textbf{Logica di Controllo adottata:}
            \begin{itemize}
                \item \textbf{SMC (Takeoff/Recovery):} Scelto per la gestione delle non-linearità e la reiezione dei disturbi in hovering.
