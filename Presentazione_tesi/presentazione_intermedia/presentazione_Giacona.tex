...
\begin{frame}{Architettura Gerarchica: Full Cascaded SMC}
    ...
    \begin{block}{Vantaggio dell'Architettura Full SMC}
        Garantisce prestazioni superiori rispetto al PID classico in presenza di forti non-linearità.
    \end{block}
\end{frame}
...
\begin{frame}{Inversione Cinematica: Dal Loop Esterno all'Interno}
    Le forze virtuali generate dagli SMC di posizione ($F_{x,req}, F_{y,req}$) devono essere tradotte in angoli di assetto.
...
\begin{frame}{Inner Loop: Stabilizzazione d'Assetto}
    ...
    \item Attraverso lo SMC vengono generati dei momenti torcenti lungo gli assi $x, y, z$ 
...
\begin{frame}{Motor Mixing Algorithm: Latero-Direzionale}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{equation}
                \Delta T_{des} = \frac{M_{x,req}}{2 d_{my}}
            \end{equation}
            \begin{equation}
                \begin{cases}
                    T_{1} = \frac{T_{front}}{2} - \Delta T_{des}\\
                    T_{2} = \frac{T_{front}}{2} + \Delta T_{des}
                \end{cases}
            \end{equation}
        \end{column}
        \begin{column}{0.48\textwidth}
...
\begin{frame}{Schema a Blocchi: Crociera}
    \begin{figure}
        ...
        \resizebox{0.75\textwidth}{!}{
            \includegraphics{controllo_orizzontale/schema_blocchi_H_t.png}
        }
...
\begin{frame}{Outer Loop: Quota e Velocità}
    L'Outer Loop genera i riferimenti per l'Inner Loop. La strategia di controllo è di tipo Fixed-Wing: la quota è controllata dall'assetto longitudinale (Pitch), mentre la velocità è controllata dalla spinta.
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Controllo Quota ($z$) $\rightarrow$ Pitch ($\theta_{ref}$)}
                Il mantenimento dell'altitudine avviene modulando l'angolo di beccheggio.
                
                \vspace{0.2cm}
                \textbf{Generazione Riferimento:}
                \begin{equation}
                    \theta_{des} = \text{PID}_z (e_z, \dot{e}_z)
                \end{equation}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Controllo Velocità ($v_x$) $\rightarrow$ Spinta ($F_{x}$)}
                Inseguimento del riferimento di velocità con compensazione aerodinamica.
                
                \vspace{0.2cm}
                \textbf{Forza Richiesta:}
                \begin{equation}
                    F_{x,req} = \text{PI}_{v_x}(e_{v_x}) + \underbrace{D_{tot}(v)}_{\text{Drag Feedforward}}
                \end{equation}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Inner Loop: Thrust Vectoring come Superfici Virtuali}
    In assenza di superfici aerodinamiche mobili, il mixer emula le superfici di controllo classiche usando i motori.
...
\begin{frame}{Motor Mixing Algorithm: Thrust}
    Il Mixer mappa i comandi di assetto sui motori (funzione di \textit{Thrust Vectoring}).

    \vspace{0.3cm}
    \textbf{Assegnazione Spinta (Propulsione)}
    ...
    \vspace{0.3cm}
    Il controllo di quota è garantito indirettamente: il servo picchia ($u_{pitch}$), il drone alza il naso ($\theta$), l'ala genera più portanza.
\end{frame}
...
\begin{frame}{Motor Mixing Algorithm: Leggi di Attuazione}
    ...
    \begin{columns}[c, onlytextwidth] 
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{equation}
                \begin{cases}
                    \theta_{1} = u_{\theta} - u_{\phi} \\
                    \theta_{2} = u_{\theta} + u_{\phi}
                \end{cases}
            \end{equation}
        \end{column}
...
\begin{frame}{Conclusioni e Sviluppi Futuri}
    ...
    \begin{itemize}
        ...
        \item Analisi di stabilità tramite linearizzazione.
    \end{itemize}
\end{frame}
