    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{equation*}
                ...
                % \bm{F}_{\text{AERO}} \propto \frac{1}{2}\rho S v^2 \cdot \begin{bmatrix} C_D(\alpha) \\ 0 \\ C_L(\alpha) \end{bmatrix}
            \end{equation*}
            \vspace{0.1cm}
            \centering
...
\begin{frame}{Regolarizzazione e Boundary Layer}
    ...
    Si introduce un \textbf{boundary layer} $\Phi$ definendo il controllo:
    \begin{equation*}
        u = -k \tanh\left( \frac{\sigma}{\Phi} \right)
    \end{equation*}

    Affinché lo strato limite sia attrattivo, deve valere $\dot{V} < 0$ sulla frontiera $|\sigma| = \Phi$:
    \begin{equation*}
        \dot{V} \le -\Phi \left( k \tanh(1) - \Delta_{max} \right) < 0
        \implies k > \frac{\Delta_{max}}{\tanh(1)}
    \end{equation*}

    \begin{block}{Ultimate Boundedness}
    ...
    \begin{columns}[c] 
        \begin{column}{0.6\textwidth}
            \begin{block}{Controllo Laterale ($y$) $\rightarrow$ Yaw ($\psi_{des}$)}
                Il controllore laterale calcola un angolo di imbardata di riferimento con un PD.
                \begin{equation}
